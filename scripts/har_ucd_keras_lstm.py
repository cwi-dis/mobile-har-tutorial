...
#pickle.dump(segments, open( "segments_90.p","wb"))
#pickle.dump(labels, open( "labels_90.p","wb"))
...
for layer in model.layers:
    print(layer.name)
print trainX.shape
model.fit(np.expand_dims(trainX,1),np.expand_dims(trainY,1), validation_split=1-trainSplitRatio,epochs=Epochs,batch_size=batchSize,verbose=2)
...
model.save('model_ucd_lstm.h5')
model.save_weights('model_weights_ucd_lstm.h5')
np.save('groundTruth_ucd_lstm.npy',np.expand_dims(testY,1))
np.save('testData_ucd_lstm.npy',np.expand_dims(testX,1))
