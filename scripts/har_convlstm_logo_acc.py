...
df = pd.DataFrame({'subject':subject_id,'acc_x':acc_x,'acc_y':acc_y,'acc_z':acc_z,'activity':act_label})


df = df[['subject','acc_x', 'acc_y', 'acc_z','activity']]
...
# plt.show()
plt.savefig('./plots/acc_plot_logo_accel.pdf', bbox_inches='tight')
plt.close()
...
plt.legend(['train', 'validation'], loc='upper left')
# plt.show()
plt.savefig('./plots/loss_plot_accel.pdf', bbox_inches='tight')

## Save your model!
model.save('./data/accel/model_had_lstm_accel.h5')
model.save_weights('./data/accel/model_weights_had_lstm_accel.h5')
np.save('./data/accel/groundTruth_had_lstm_accel.npy',np.expand_dims(testY,1))
np.save('./data/accel/testData_had_lstm_accel.npy',np.expand_dims(testX,1))

## write to JSON, in case you wanrt to work with that data format later when inspecting your model
with open("./data/accel/model_had_lstm_accel.json", "w") as json_file:
    json_file.write(model.to_json())

## write cvscores to file
with open('./data/accel/cvscores_convlstm_accel.txt', 'w') as cvs_file:
    cvs_file.write("LeaveOneGroupOut cross-validation scores: %.2f%% (+/- %.2f%%)" % (np.mean(cvscores), np.std(cvscores)))
